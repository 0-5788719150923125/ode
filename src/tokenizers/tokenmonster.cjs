...
class TokenMonster {
    ...
    Decoder() {
        ...
        return new (class Decoder {
            ...
            detokenize(tokens) {
                ...
                this.remainder = decoded.slice(
                    decoded.length - invalidBytes,
                    decoded.length
                )
                let decodedString = decoder.decode(
                    decoded.slice(0, decoded.length - invalidBytes)
                )
                if (parent.capcode) {
                    ...
                    protocol
                        .get(url, (response) => {
                            if (
                                ...
                                reject(
                                    new Error(
                                        `Failed to fetch the URL (${response.statusCode}): ${url}`
                                    )
                                )
                            }
                            const chunks = []
    ...
    tokenize_bytes(text) {
        ...
        outerLoop: while (i < textLen) {
            ;[id, len, found] = this.word2index.findLargestSubarray(
                text.subarray(i, i + Math.min(textLen - i, this.max_token_len))
            )
            ...
            if (found) {
                while (i < textLen) {
                    ...
                    if (i2 < textLen && (temp1 == 0 || temp2 != 12)) {
                        ...
                        ;[id2, len2, found2] =
                            this.word2index.findLargestSubarray(
                                text.subarray(
                                    i2,
                                    i2 +
                                        Math.min(
                        ...
                        if (found2) {
                            ...
                            if (
                                ...
                                testlen = Math.min(
                                    textLen - i2,
                                    this.max_token_len - this.charset
                                )
                                lilbuf.set(
                                    text.subarray(i2, i2 + testlen),
                                    this.charset
                                )
                                ;[id2b, len2b, found2b] =
                                    this.word2index.findLargestSubarray(
                        ...
                        if (alternativeid != 16777215) {
                            // Get alternative 1
                            slen =
                                this.index2alternative_length[id] -
                                forwardDelete
                            i3 = i + slen
                            ;[id3, len3, found3] =
                                this.word2index.findLargestSubarray(
                            ...
                            if (found3) {
                                ...
                                branch2 = slen + len3
                                nWords =
                                    this.index2nWords[alternativeid] -
                                    forwardDelete
                            ...
                            if (alternative2id != 16777215) {
                                slen2 =
                                    this.index2alternative2_length[id] -
                                    forwardDelete
                                i4 = i + slen2
                                ;[id4, len4, found4] =
                                    this.word2index.findLargestSubarray(
                                ...
                                if (found4) {
                                    branch3 = slen2 + len4
                                    nWords =
                                        this.index2nWords[alternative2id] -
                                        forwardDelete
                                    ...
                                    if (
                                        ...
                                        if (found4b && len4b > len4 + 1) {
                                            ...
                                            nWords2 =
                                                this.index2nWords[id4b] - 1
                                            flag2 = this.index2flag[id4b]
                                            beginByte =
                                                this.beginByte[text[i4 + len4b]]
                                            score3b =
                                                branch3b +
                                                (flag >> 7) +
                                                (flag2 >> 7) +
...
function incompleteUTF8Bytes(bytes) {
    ...
    if ((bytes[bytesLen - 1] & 0b10000000) === 0) return 0
    ...
    if (bytesLen - seqStart < seqLen) return bytesLen - seqStart
    // If the sequence start byte was not the start of a multi-byte sequence, then the array is incomplete.
    if (seqLen === 1 && (bytes[seqStart] & 0b11000000) !== 0) return bytesLen
    return 0
}
...
function incompleteUTF16Bytes(bytes) {
    ...
    if (bytesLen % 2 !== 0) {
        let lastThreeBytes =
            bytesLen >= 3
                ? bytes[bytesLen - 2] | (bytes[bytesLen - 3] << 8)
                : null
            ...
            if (isUpper(r)) {
                ...
                if (isLower(r)) {
                    ...
                    if (isNumber(r)) {
                        ...
                        !(r == apostrophe || r == apostrophe2 || isModifier(r))
                    ) {
                        inWord = false
                    }
                }
            ...
            if (isLower(r)) {
                if (
                    !(
                        ...
                        isLetter(rlast) ||
                        (isLetter(rlast2) &&
                            (rlast == apostrophe || rlast == apostrophe2)) ||
                        isModifier(rlast)
                    )
                ) {
                    buf[pos++] = deleteToken
